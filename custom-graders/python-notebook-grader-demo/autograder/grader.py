...
def main():
    ...
    # The destination for the submission to be created
    submission_destination = "grader/submission"
    ...
    try:
        make_submission(submission_location + nb_output, graded_id, package_id, submission_destination)
    ...
    try:
        ...
        with stdout_redirected():
            from submission import factors, rotation_matrix
    except Exception as e:
    ...
    feedback_flags = {"factors":0, "rotation_matrix":0}

    try:
        #stdout_redirected prevents print statements from learner submission
        #from being stored in stdout
        with stdout_redirected():
            learnerOutput = [factors(x) for x in testCases["factors"]["input"]]
    except Exception as e:
        send_feedback(0.0, "Your code returned this error: " + str(e))
        return

    for j in range(0,numTestCases):
        if testCases["factors"]["output"][j] != learnerOutput[j]:
            numTestCasesFailed += 1
            feedback_flags["factors"] = 1

    try:
        #stdout_redirected prevents print statements from learner submission
        #from being stored in stdout
        with stdout_redirected():
            learnerOutput = [rotation_matrix(x) for x in testCases["rotation_matrix"]["input"]]
    except Exception as e:
        send_feedback(0.0, "Your code returned this error: " + str(e))
        return

    for j in range(0,numTestCases):
        if not np.allclose(testCases["rotation_matrix"]["output"][j], learnerOutput[j], equal_nan=False):
            numTestCasesFailed += 1
            feedback_flags["rotation_matrix"] = 1
    ...
    if numTestCasesFailed > 0:
        ...
        for flag in feedback_flags:
            if feedback_flags[flag]:
                feedback += " Test case(s) failed in " + flag + "."
